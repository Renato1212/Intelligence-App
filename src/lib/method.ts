...
export const METHOD_STEPS: MethodStep[] = [
  ...
  },
  {
    ...
    route: '/brief?step=environment&panel=cal',
  ...
  },
  {
    ...
    route: '/brief?step=environment&panel=macro',
  ...
  },
  {
    ...
    route: '/brief?step=environment&panel=opt',
  ...
  },
  {
    ...
    route: '/brief?step=environment&panel=pos',
...
export const CONNECTS: Record<string, Connection[]> = {
  catalysts: [
    { route: '/brief?step=environment&panel=opt', label: 'Options & Vol', why: 'the expected move + gamma regime tell you how far the print can run and whether to fade or follow it' },
    { route: '/brief?step=environment&panel=pos', label: 'Market Intel', why: 'a print that hits a positioning extreme is the squeeze setup — check the percentile before fading anything' },
    { route: '/', label: 'Session Clock', why: 'the same release trades differently at the open vs lunch — know your window' },
  ],
  optvol: [
    { route: '/brief?step=environment&panel=cal', label: 'Catalysts', why: 'event premium in the VIX curve points at a specific date — find it on the calendar' },
    { route: '/brief?step=environment&panel=macro', label: 'Macro Map', why: 'gamma says HOW price moves; the narrative and cross-asset reads say WHICH WAY to lean' },
    { route: '/', label: 'Session Clock', why: 'walls matter most into the close and OPEX hours — time-stamp your levels' },
    { route: '/brief?step=technicals&panel=charts', label: 'Charts', why: 'draw these levels on the TradingView chart and watch how price treats them live' },
  ],
  macro: [
    { route: '/brief?step=environment&panel=pos', label: 'Market Intel', why: 'a narrative surge + a positioning extreme in the same market is the crowded-story setup' },
    { route: '/brief?step=environment&panel=cal', label: 'Catalysts', why: 'the surging story decides which of this week\'s prints actually matters' },
    { route: '/brief?step=environment&panel=opt', label: 'Options & Vol', why: 'regime context + dealer levels = direction AND location' },
  ],
  intel: [
    { route: '/brief?step=environment&panel=cal', label: 'Catalysts', why: 'positioning is fuel; the scheduled print is the spark — line them up' },
    { route: '/debrief', label: 'Preparation', why: 'focus-list markets carry their positioning read into your prep automatically' },
  ],
  sessions: [
    { route: '/brief?step=environment&panel=opt', label: 'Options & Vol', why: 'the session flow map and the dealer levels are the same mechanics seen from time vs price' },
    { route: '/archive?step=edge', label: 'Edge Analytics', why: 'your timing heatmap shows which of these windows actually pays you' },
  ],
  tradedesk: [
    { route: '/brief?step=fundamentals', label: 'Edge Terminal', why: 'read the regime and the rails on the Terminal BEFORE you stage an order here' },
    { route: '/debrief', label: 'Preparation', why: 'your IF-THEN plan and risk limit live here — execute the plan, do not improvise at the ticket' },
    { route: '/plan?step=size', label: 'Risk Guardrail', why: 'know today\'s loss limit and safe size before you arm live trading' },
  ],
  flows: [
    { route: '/brief?step=environment&panel=macro', label: 'Macro Map', why: 'the yield curve and narrative layer explain WHY the ratios are moving' },
    { route: '/brief?step=environment&panel=pos', label: 'Market Intel', why: 'a flow rotation into a market where specs are already max long is the squeeze setup' },
    { route: '/plan?step=ideas', label: 'Conviction Board', why: 'cross-asset agreement or conflict feeds straight into an idea\'s conviction score' },
    { route: '/brief?step=fundamentals', label: 'Edge Terminal', why: 'the regime read and the flow read should agree — when they do not, trade smaller' },
  ],
  profile: [
    { route: '/plan?step=ideas', label: 'Conviction Board', why: 'the auction read becomes concrete setups — poor extremes, balance fades, trapped-side continuation' },
    { route: '/debrief', label: 'Trade Desk', why: 'these levels are where the DOM matters: watch the tape defend value and the extremes on the ladder' },
    { route: '/debrief', label: 'Preparation', why: 'send the read straight into your prep so the plan is written before the open' },
    { route: '/brief?step=technicals&panel=charts', label: 'Charts', why: 'draw POC, value and the IB on the chart and watch how price treats them live' },
  ],
  ideas: [
    { route: '/brief?step=fundamentals', label: 'Edge Terminal', why: 'the regime read is the backdrop every idea trades inside — check they agree before sizing' },
    { route: '/brief?step=environment&panel=cal', label: 'Catalysts', why: 'data-domain ideas fire around scheduled prints — the deep print study and playbook live here' },
    { route: '/debrief', label: 'Trade Desk', why: 'when a trigger prints, stage the order on the DOM and let auto-record capture the execution' },
    { route: '/debrief', label: 'Preparation', why: 'copy the idea\'s trigger/invalidation into your IF-THEN plan — the board proposes, your prep disposes' },
  ],
  terminal: [
    { route: '/brief?step=environment&panel=cal', label: 'Catalysts', why: 'the Terminal names today\'s scheduled risk — the deep print study and playbook live here' },
    { route: '/brief?step=environment&panel=opt', label: 'Options & Vol', why: 'the regime and rails on the Terminal come from this section\'s full gamma/vol machinery' },
    { route: '/debrief', label: 'Preparation', why: 'turn the Terminal read into IF-THEN statements before the open — the read is input, the plan is the edge' },
  ],
  charts: [
    { route: '/brief?step=environment&panel=opt', label: 'Options & Vol', why: 'draw the put wall, call wall, flip and expected-move band on the chart — dealer levels are the ones that hold' },
    { route: '/brief?step=environment&panel=cal', label: 'Catalysts', why: 'time-stamp the chart with today\'s release times; the candle AT the print is the regime tell' },
    { route: '/', label: 'Session Clock', why: 'the chart renders in Lisbon time — line up the session opens and the prime-time overlap' },
  ],
};
